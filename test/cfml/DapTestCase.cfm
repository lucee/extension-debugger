...
function triggerArtifact( required string filename, struct params = {}, boolean allowErrors = false ) {
	...
	thread name="#variables.httpThread#" requestUrl=requestUrl httpResult=variables.httpResult allowErrors=arguments.allowErrors {
		...
		try {
			...
			systemOutput( "triggerArtifact: done #attributes.requestUrl# status=#local.r.statusCode# elapsedMs=#getTickCount() - httpStart#", true );
		} catch ( any e ) {
			systemOutput( "triggerArtifact HTTP error: #attributes.requestUrl# msg=#e.message# elapsedMs=#getTickCount() - httpStart#", true );
			systemOutput( e, true );
...
// Nuclear by default: clears every path the client has ever set a breakpoint on.
// Used from afterEach to prevent stale BPs from one test blocking another
// test's newInstance() via <clinit>/constructor line hits.
function clearBreakpoints( string path = "" ) {
	variables.dap.clearAllKnownBreakpoints();
}
...
// Assert a path ends with the given suffix, normalising separators
// so the same expected suffix ("udfInclude/inner.cfm") works on both
// Windows (backslashes) and Linux (forward slashes).
function _expectPathEndsWith( required string actual, required string suffix, string message = "" ) {
	var normalized = replace( arguments.actual, "\", "/", "all" );
	var msg = len( arguments.message )
		? arguments.message
		: "Expected path '#arguments.actual#' to end with '#arguments.suffix#'";
	expect( right( normalized, len( arguments.suffix ) ) ).toBe( arguments.suffix, msg );
}
</cfscript>
