...
import org.lucee.extension.debugger.LuceeTransformer.ClassInjection;
import org.lucee.extension.debugger.Version;
...
public class Agent {
    ...
    private static class CoreInjectionLinearization {
        private static Map<String, Integer> linearizedCoreInjectClasses() {
            ...
            // Utility classes - no hierarchies, all order 0
    ...
    public static void premain(String argString, Instrumentation inst) throws Throwable {
        ...
        try (var jarFile = new JarFile(parsedArgs.jarPath)) {
            ...
            var classInjections = jarFile
                .stream()
                .filter(jarEntry -> !jarEntry.isDirectory() && (jarEntry.getName().startsWith("org/lucee/extension/debugger/coreinject") || jarEntry.getName().startsWith("org/lucee/extension/debugger/util")) && jarEntry.getName().endsWith(".class"))
                .map(jarEntry -> {
