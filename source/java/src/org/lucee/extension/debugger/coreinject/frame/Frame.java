        ...
        if (!closureScopeGloballyDisabled) {
            ...
            try {
                for (int i = 0; i < captureChainLen; i++) {
                    ...
                    // Cast via Object to bypass compile-time check - the shim interface is added via bytecode instrumentation at runtime
                    checkedPutScopeRef("captured local " + i, ((ClosureScopeLocalScopeAccessorShim)(Object)scopeChain.get(i)).getLocalScope());
                }
            }
