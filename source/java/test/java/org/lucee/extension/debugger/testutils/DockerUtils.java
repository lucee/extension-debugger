...
public class DockerUtils {
	...
	static File mungeDockerfile(File dockerFile) {
		try {
			var s = new String(Files.readAllBytes(dockerFile.toPath()))
				.replaceAll("@LUCEEDEBUG_JAR", "debugger-agent-" + Version.VERSION + ".jar");
	...
	public static ContainerID getFreshDefaultContainer(
		...
		// Maven output is in agent/target/ instead of Gradle's build/libs/
		hostConfig.setBinds(
			new Bind(
				Paths.get(projectRoot.toString(), "agent/target/").toString(),
				new Volume("/build/")
			),
			new Bind(
