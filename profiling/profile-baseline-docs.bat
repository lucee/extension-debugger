...
rem ============================================================
rem Profile luceedebug - BASELINE using lucee-docs build
rem ============================================================
...
echo.
echo ============================================================
echo BASELINE PROFILE - lucee-docs build without luceedebug
echo ============================================================
echo.

call ant -buildfile "%SCRIPT_RUNNER%" -DluceeVersionQuery="7/all/jar" -Dwebroot="%DOCS_DIR%" -Dexecute="build.cfm" -Dextensions="%EXTENSIONS%" -DFlightRecording="true" -DFlightRecordingFilename="%OUTPUT_DIR%\baseline-docs.jfr" -DFlightRecordingSettings="%JFC_CONFIG%" -DuniqueWorkingDir="true" -DpostCleanup="false" > "%OUTPUT_DIR%\baseline-docs-output.txt" 2>&1

echo.
echo Results saved to: %OUTPUT_DIR%\baseline-docs-output.txt
echo JFR saved to: %OUTPUT_DIR%\baseline-docs.jfr
echo.

type "%OUTPUT_DIR%\baseline-docs-output.txt" | findstr /C:"Total time:" /C:"BUILD"
